                ...
                <c:choose>
                    <c:when test="${not empty isOrder}">
                        <c:if test="${not empty customers}">
                            <select name="customerID" class="form-select w-auto" style="min-width: 200px;">
                                ...
                                <c:forEach items="${customers}" var="cus">
                                    <option value="${cus.ID}" ${customerID==cus.ID ? 'selected' : '' }>${cus.fullname}
                                        (${cus.username})</option>
                                </c:forEach>
                            </select>
                        </c:if>
                        <select name="status" class="form-select w-auto">
                            ...
                            <option value="4" ${status=='4' ? 'selected' : '' }>Đã thanh toán</option>
                            <option value="8" ${status=='8' ? 'selected' : '' }>Đã duyệt</option>
                            <option value="5" ${status=='5' ? 'selected' : '' }>Đã chuẩn bị hàng</option>
                            <option value="6" ${status=='6' ? 'selected' : '' }>Đã giao ĐVVC</option>
